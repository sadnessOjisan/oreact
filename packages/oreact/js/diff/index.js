...
export function diff(arg) {
  ...
  let tmp,
      newType = newVNode.type; // createElement は constructor を undefined で設定するtので、そこ経由で作られたことを保証する。

  if (newVNode.constructor !== undefined) return null; // newVNode がコンポーネントかエレメントかで処理が分岐

  if (typeof newType == 'function') {
    // newVNode がコンポーネントの時の分岐
    let c, isNew, oldProps, oldState;
    let newProps = newVNode.props;
    let componentContext = EMPTY_OBJ; // コンポーネントオブジェクトの作成
    ...
    oldProps = c.props;
    oldState = c.state; // 差分更新前(diff中)に呼び出されるライフサイクルイベントを実行
    ...
    tmp = c.render(c.props); // Handle setState called in render, see #2553
    ...
    newVNode._dom = diffElementNodes({
      dom: oldVNode._dom,
      // この分岐に入るとparentDomではなくoldVNode._domを見るようになる。(階層を下る)
      newVNode: newVNode,
      oldVNode: oldVNode,
      excessDomChildren: excessDomChildren,
      commitQueue: commitQueue
...
function diffElementNodes(arg) {
  ...
  if (dom == null) {
    ...
    dom = document.createElement(newVNode.type, newProps.is && {
      is: newProps.is
  ...
  if (newVNode.type === null) {
    ...
    const props = oldVNode.props || EMPTY_OBJ; // VNodeの差分を取る。domは破壊的操作がされる

    diffProps(dom, newProps, props); // VNode の children に diff を取るためにchildrenを抽出

    i = newVNode.props.children; // newVNodeがComponentの入れ子でなくてもElementの入れ子の可能性があるので、childrenの比較も行う
